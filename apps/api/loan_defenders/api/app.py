...
try:
    ...
    # Path goes from app.py -> api -> loan_defenders -> apps -> api -> root
...
from loan_defenders.api.config import settings
from loan_defenders.api.models import (
    ConversationRequest,
...
from loan_defenders.api.session_manager import session_manager
...
async def handle_unified_chat(request: ConversationRequest) -> ConversationResponse:
    ...
    try:
        ...
        if session.state_machine is None:
            from loan_defenders.orchestrators.conversation_state_machine import ConversationStateMachine
